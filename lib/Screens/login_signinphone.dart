...
import 'package:heybuddy/Screens/login_otpscreen.dart';
...
class LoginSignInPhone extends StatefulWidget {
  const LoginSignInPhone({Key? key}) : super(key: key);

  @override
  _LoginSignInPhoneState createState() => _LoginSignInPhoneState();
}

class _LoginSignInPhoneState extends State<LoginSignInPhone> {
  String dialCodeDigits = "+91";
  TextEditingController _controller = TextEditingController();

  var output;
  checking() async {
    output = await CheckUserExist.usercheck(_controller.text);
    print("status is $output");
    return output;
  }

  submit(BuildContext context) async {
    var output = await CheckUserExist.usercheck(_controller.text);
    print("status is $output");
    // await checking();
    if (_controller.text.isEmpty || _controller.text.length < 5) {
      ...
      const snackBar = SnackBar(
        content: Text("User Not Exist Please SignUp !"),
        duration: Duration(milliseconds: 2000),
        backgroundColor: text6,
      );
      ScaffoldMessenger.of(context).showSnackBar(snackBar);
